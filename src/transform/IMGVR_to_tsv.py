...
def parse(subject_index, df, debug=False):
    edges = []
    nodes = []
    dims = df.shape
        ...
        for j in range(0, len(object_fields)):
            ...
            if not pd.isnull(addval) and addval != np.NAN:
                ...
                if "GOLD Sequencing Project ID" == object_fields[j]:
                    ...
                    newstr = object_field_prefixes[subject_index] + ":" + str(
                        df.iloc[i, subject_index]) + "\tbiolink:has_attribute\t" + \
                    ...
                    newstr = subject_field_prefix + ":" + str(df.iloc[i, subject_index]) + "\t" + object_edge_labels[
                        j] + "\t" + object_field_prefixes[j] + ":" + str(addval) + "\t" + object_edge_labels[
                                 j] + "\t" + "GOLD"
                    if (debug):
                        print("adding " + newstr)
                    if newstr not in edges:
                    ...
                    node1str = subject_field_prefix + ":" + str(df.iloc[i, subject_index]) + "\t" + str(
                        df.iloc[i, subject_index]) + "\t" + subject_field_category + "\tGOLD"
                    if (debug):
                        print("adding " + node1str)
                    if node1str not in nodes:
                        nodes.append(node1str)
