...
embedding_path = "./IMGRVR/SkipGram_embedding_IMGVR_sample.npy"

graph = EnsmallenGraph.from_unsorted_csv(
    edge_path="/global/scratch/marcin/N2V/embiggen/notebooks/IMGRVR/IMGVR_sample_KGX_edges.tsv",
...
for perplexity in tqdm(perplexities, desc="Perplexities", leave=False):
    tsne_path = f"tnse_IMGRVR_merged/IMGRVR_merged_node_{perplexity}"
...
def load_graph(node_type_column: str, default_node_type: str = "unknown"):
    ...
    return EnsmallenGraph.from_unsorted_csv(
        ...
        node_path="/global/scratch/marcin/N2V/embiggen/notebooks/IMGRVR/IMGVR_sample_KGX_nodes.tsv",
        nodes_column="id",
...
for node_types_column, axes_row in zip(node_types_columns, axes_matrix):
    for perplexity, axes in zip(perplexities, axes_row):
        tsne_embedding = np.load(f"tnse_IMGRVR_merged/IMGRVR_merged_node_{perplexity}.npy")
        plot_embedding(
...
###SAVE
plt.savefig('IMGRVR_merged_node_types.png')
#plt.show()
...
for perplexity, axes in zip(perplexities, axes_row):
    ...
    axes.set_title(f"Perp. {perplexity}")
###SAVE
plt.savefig('IMGRVR_merged_degrees.png')
#plt.show()
