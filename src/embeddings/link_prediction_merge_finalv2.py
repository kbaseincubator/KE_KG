...
graph = EnsmallenGraph.from_unsorted_csv(
    edge_path="./IMGVR/merged_final/IMGVR_merged_final_KGX_edges.tsv",
    node_path="./IMGVR/merged_final/IMGVR_merged_final_KGX_nodes.tsv",
...
neg_training, neg_validation = graph.merged_final_negatives(
    random_state=seed,
...
def task_generator(
    ...
    """
    for path in tqdm(glob("./IMGVR/merged_final/*embedding.npy"), desc="Embedding"):
...
os.makedirs("link_prediction_merge_final_v2", exist_ok=True)

for embedding_model, method, train, valid in task_generator(pos_training, pos_validation, neg_training, neg_validation):
    history_path = f"link_prediction_merge_final_v2/{embedding_model}_{method}.csv"
...
metrics_reports_path = "linear_models_reports.csv"
...
metrics_reports_path = "link_prediction_merge_final_v2/linear_models_reports.csv"
if os.path.exists(metrics_reports_path):
    metrics_reports = pd.read_csv(metrics_reports_path)
else:
    metrics_reports = []
...
all_reports.to_csv("link_prediction_merge_final_v2/all_reports.csv", index=None)
...
barplots(
    ...
    path = 'barplots_merge_final_v2/{feature}.jpg',
...
for barplot_path in glob("barplots_merge_final_v2/*"):
